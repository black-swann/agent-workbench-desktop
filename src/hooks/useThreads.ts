...
type ThreadAction =
  ...
  | { type: "clearWorkspaceSessionState"; workspaceId: string }
...
function threadReducer(state: ThreadState, action: ThreadAction): ThreadState {
  switch (action.type) {
    ...
    case "clearWorkspaceSessionState": {
      const workspaceThreads = state.threadsByWorkspace[action.workspaceId] ?? [];
      const threadIds = new Set(workspaceThreads.map((thread) => thread.id));
      const nextStatus = { ...state.threadStatusById };
      const nextTurns = { ...state.activeTurnIdByThread };
      for (const threadId of threadIds) {
        const existing = nextStatus[threadId];
        if (existing) {
          nextStatus[threadId] = {
            ...existing,
            isProcessing: false,
        ...
        nextTurns[threadId] = null;
      }
      return {
        ...state,
        approvals: state.approvals.filter(
          (approval) => approval.workspace_id !== action.workspaceId,
        ),
        threadStatusById: nextStatus,
        activeTurnIdByThread: nextTurns,
        rateLimitsByWorkspace: {
    ...
    case "appendAgentDelta": {
      ...
      if (index >= 0 && list[index].kind === "message") {
        const existing = list[index] as MessageItem;
        list[index] = {
          ...existing,
    ...
    case "appendReasoningSummary": {
      ...
      const updated: ReasoningItem = {
        ...base,
        summary: `${base.summary}${action.delta}`,
      };
    ...
    case "appendReasoningContent": {
      ...
      const updated: ReasoningItem = {
        ...base,
        content: `${base.content}${action.delta}`,
      };
      const next = index >= 0 ? [...list] : [...list, updated];
      if (index >= 0) {
        next[index] = updated;
    ...
    case "appendToolOutput": {
      ...
      const existing = list[index] as ToolItem;
...
type UseThreadsOptions = {
  ...
  onWorkspaceDisconnected?: (id: string, reason: string) => void;
...
function parseReviewTarget(input: string) {
  ...
  return {
    type: "custom",
    instructions: rest || "Review the recent work in this thread.",
  } as const;
}
...
function buildItemsFromThread(thread: Record<string, unknown>) {
  ...
  turns.forEach((turn) => {
    ...
    turnItems.forEach((item: unknown) => {
...
export function useThreads({
  ...
  const [isListingByWorkspace, setIsListingByWorkspace] = useState<
    Record<string, boolean>
  >({});
  const [isResumingByThread, setIsResumingByThread] = useState<
    Record<string, boolean>
  >({});
  ...
  const handlers = useMemo(
    () => ({
      ...
      onWorkspaceDisconnected: (workspaceId: string, reason: string) => {
        dispatch({ type: "clearWorkspaceSessionState", workspaceId });
        const workspaceThreads = state.threadsByWorkspace[workspaceId] ?? [];
        for (const thread of workspaceThreads) {
          loadedThreads.current[thread.id] = false;
        }
        setIsListingByWorkspace((prev) => ({ ...prev, [workspaceId]: false }));
        onWorkspaceDisconnected?.(workspaceId, reason);
      },
      onApprovalRequest: (approval: ApprovalRequest) => {
        dispatch({ type: "addApproval", approval });
      },
      onAppServerEvent: (event: { message?: Record<string, unknown> }) => {
      ...
      onItemStarted: (workspaceId: string, threadId: string, item: unknown) => {
        dispatch({ type: "ensureThread", workspaceId, threadId });
        ...
        const converted = buildConversationItem(item as Record<string, unknown>);
        if (converted) {
          dispatch({ type: "upsertItem", threadId, item: converted });
        }
      },
      onItemCompleted: (workspaceId: string, threadId: string, item: unknown) => {
        dispatch({ type: "ensureThread", workspaceId, threadId });
        const itemType = asString((item as Record<string, unknown>)?.type ?? "");
        if (itemType === "enteredReviewMode") {
          dispatch({ type: "markReviewing", threadId, isReviewing: true });
  ...
  const resumeThreadForWorkspace = useCallback(
    async (workspaceId: string, threadId: string, force = false) => {
      ...
      setIsResumingByThread((prev) => ({ ...prev, [threadId]: true }));
      onDebug?.({
      ...
      try {
        ...
        setIsResumingByThread((prev) => ({ ...prev, [threadId]: false }));
      }
    },
    [onDebug, onError],
  );
  ...
  const listThreadsForWorkspace = useCallback(
    async (workspace: WorkspaceInfo) => {
      setIsListingByWorkspace((prev) => ({ ...prev, [workspace.id]: true }));
      onDebug?.({
  ...
  const sendUserMessage = useCallback(
    async (text: string) => {
      ...
      try {
        const response: any = await sendUserMessageService(
          activeWorkspace.id,
          threadId,
          messageText,
